...
import com.example.basemodule.navigation.resultListener
...
fun LoginScreen(
    ...
    BaseScreen(
        viewModel = viewModel,
        onCreate = { resultManager ->
            Log.d("TAG", "LoginScreen: onCreate")
            resultManager.resultListener(AuthNavModule.LoginScreen::class.java.name) {
                it.getString("error")?.let {
                    viewModel.onAction(LoginAction.OnLoginError(it))
                }
            }
        }
    ) {
